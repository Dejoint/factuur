...
function getCompany(array $lijst, int $id) : array {
        return $lijst[$id];
}

/*function getCompanyOV(array $lijst, int $zip) : array {
        $resultaat = [];
        foreach ($companies as $company) {
                if (stripos($company['zip'],9***) !== false) {
                        $resultaat[] = $company;
                }
        }
        return $resultaat;
...
*/
function getCompaniesBySearch(array $companies, string $term) : array {
        $resultaat = [];
        foreach ($companies as $company) {
                if (stripos($company['name'], $term) !== false) {
                        $resultaat[] = $company;
                }
        }
        return $resultaat;
}/*
function getCompaniesBySort(array $companies, string $sort) : array{
        $resultaat = [];
        switch ($sort) {
                case 'name':
                        $resultaat = asort(array);
                        break;
                case 'address'
...
if (array_key_exists('id', $_GET)) {
        $index = $_GET['id'];
        $resultaat = [getCompany($companies, $index)];
} else if (array_key_exists('search', $_GET)) {
        $term = $_GET['search'];

        $resultaat = getCompaniesBySearch($companies, $term);
} else if (array_key_exists('sort', $_GET)) {
        $sort = $_GET['sort'];
        $resultaat = getCompaniesBySort($companies, $sort);
else {
        $resultaat = $companies; 
};
        ...
        <div class="table-wrapper">
            <table class="alt">
                ...
                <?php
                        ...
                        foreach ($resultaat as $bedrijf) {?>
                                <tr>
                                        <td> <?php echo $bedrijf['name']?></td>
                                        <td> <?php echo $bedrijf['address']?></td>
                                        <td><?php echo $bedrijf['zip'] . ' ' . $bedrijf['city'] ?></td>
                        <?php }; ?>
                                </tr>
                </tbody>
            </table>
